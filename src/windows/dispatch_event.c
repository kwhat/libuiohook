...
bool dispatch_mouse_wheel(MSLLHOOKSTRUCT *mshook, uint8_t direction) {
    ...
    uio_event.data.wheel.type = get_scroll_wheel_type();
    uio_event.data.wheel.amount = get_scroll_wheel_amount();
    ...
    uio_event.data.wheel.rotation = (int16_t) HIWORD(mshook->mouseData) / WHEEL_DELTA;
