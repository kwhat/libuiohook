...
static void initialize_modifiers() {
    ...
    if (GetKeyState(VK_RCONTROL)) { set_modifier_mask(MASK_CTRL_R);     }
    if (GetKeyState(VK_LMENU))    { set_modifier_mask(MASK_ALT_L);      }
    if (GetKeyState(VK_RMENU))    { set_modifier_mask(MASK_ALT_R);      }
    if (GetKeyState(VK_LWIN))     { set_modifier_mask(MASK_META_L);     }
    if (GetKeyState(VK_RWIN))     { set_modifier_mask(MASK_META_R);     }

    if (GetKeyState(VK_LBUTTON))  { set_modifier_mask(MASK_BUTTON1);    }
    if (GetKeyState(VK_RBUTTON))  { set_modifier_mask(MASK_BUTTON2);    }
    if (GetKeyState(VK_MBUTTON))  { set_modifier_mask(MASK_BUTTON3);    }
    if (GetKeyState(VK_XBUTTON1)) { set_modifier_mask(MASK_BUTTON4);    }
    if (GetKeyState(VK_XBUTTON2)) { set_modifier_mask(MASK_BUTTON5);    }

    if (GetKeyState(VK_NUMLOCK)) { set_modifier_mask(MASK_NUM_LOCK);    }
    if (GetKeyState(VK_CAPITAL)) { set_modifier_mask(MASK_CAPS_LOCK);   }
    if (GetKeyState(VK_SCROLL))  { set_modifier_mask(MASK_SCROLL_LOCK); }
}
...
{
    logger(LOG_LEVEL_ERROR, "%s [%u]: Testing10: %#X.\n",
            __FUNCTION__, __LINE__, code);

    MSG* lpmsg = (MSG*)lParam;
        logger(LOG_LEVEL_ERROR, "%s [%u]: Hooked for HWND: %#X.\n",
                __FUNCTION__, __LINE__, lpmsg->hwnd);

        logger(LOG_LEVEL_ERROR, "%s [%u]: Hooked for message: %#X.\n",
                __FUNCTION__, __LINE__, lpmsg->message);
...
UIOHOOK_API int hook_run() {
    ...
    HHOOK m_msgHook = SetWindowsHookEx(WH_GETMESSAGE, message_hook_proc, NULL, GetCurrentThreadId());
    DWORD errCode = GetLastError();
        logger(LOG_LEVEL_WARN, "%s [%u]: TESTING Eroor:  %0X\n",
